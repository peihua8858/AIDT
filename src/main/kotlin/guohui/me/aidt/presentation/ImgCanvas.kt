...
class ImgCanvas(
    ...
    private inner class Listener : MouseAdapter() {
        ...
        override fun mouseWheelMoved(event: MouseWheelEvent) {
            ...
            println("mouseWheelMoved offsetX = $offsetX   offsetY = $offsetY   scale = $scale  rect.width=${rect.width}  rect.height= ${rect.height}")
            repaint()
        }
    ...
    fun zoomInImage(){
        val fScale = 0.01f
        offsetX -= (rect.width/2 - offsetX) * fScale
        offsetY -= (rect.height/2 - offsetY) * fScale
        scale *= 1.0f + fScale
        //scale += 0.1f
        repaint()
    }
